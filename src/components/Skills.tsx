  ...
  useEffect(() => {
    ...
    const initMatter = () => {
      ...
      const skillBodies = skillsData.map((skill) => {
        const baseWidth = isMobile ? 60 : 120;
        const charWidth = isMobile ? 5 : 8;
        const width = baseWidth + skill.length * charWidth;
        const height = isMobile ? 32 : 50;
        const x =
          Math.random() * (sceneRef.current!.clientWidth - width) + width / 2;
        const y = Math.random() * 200 + 50;

        const body = Bodies.rectangle(x, y, width, height, {
          chamfer: { radius: isMobile ? 16 : 25 },
          restitution: 0.5,
          friction: 0.001,
          render: {
            fillStyle: "#ffffff",
